...
pdf("Figures/coattails.pdf", height = 3, width = 9)
library(R2jags)
library(foreign)
library(arm)
...
plot(NULL, xlim = c(0, 1), ylim = c(3.5, 8), xlab = NA, ylab = NA, axes = F)
axis(side = 1, cex.axis = .9)
mtext(side = 1, "Pr(Vote Rep | Vote Bush) - Pr(Vote Rep | Vote Clinton)", line = 2, cex = .9)
...
m1 <- glm(Rep_House_vote ~ Rep_pid + Dem_pid + Rep_House_inc + Dem_House_inc + 
  ...
  family = binomial, data = nes92)
sims1 <- coef(sim(m1))
x.clinton <- c(1, 1, 0, 0, 1, 0, 1, 0,0,0,0,0,0)
sims.clinton <- plogis(sims1%*%x.clinton)
...
house.exit <- read.dta("Data/Senate_ExitPolls_1992.dta")
nes92 <- read.dta("Data/SES_1992.dta")
